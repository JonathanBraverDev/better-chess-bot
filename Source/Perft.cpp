...
void detailedPerftDivide(Position &pos, int depth) {
  ...
  Reporter::printHeader("Detailed Perft Divide (Depth " +
                        std::to_string(depth) + ")");
  ...
  for (const auto &move : moves) {
    ...
    Reporter::printPerftMove(move.getOriginIndex(), move.getDestinationIndex(),
                             moveStats.nodes);
  }
...
void runPerftTest(std::string fen, int depth, long long expectedNodes) {
  ...
  if (nodes == expectedNodes) {
    Reporter::printMessage("[PASS] Depth " + std::to_string(depth) +
                           " Nodes: " + std::to_string(nodes) +
                           " Time: " + std::to_string(elapsed.count()) + "s");
  } else {
    Reporter::printMessage("[FAIL] Depth " + std::to_string(depth) +
                           " Expected: " + std::to_string(expectedNodes) +
...
void runDetailedPerftTest(std::string fen, int depth,
  ...
  if (stats.nodes > 0 && elapsed.count() > 0) {
    Reporter::printMessage(
        "Nodes/sec: " +
        std::to_string(static_cast<long long>(stats.nodes / elapsed.count())));
  }
  ...
  if (stats.nodes != expectedStats->nodes) {
    Reporter::printMessage("  [FAIL] Nodes: expected " +
                           std::to_string(expectedStats->nodes) + ", got " +
                           std::to_string(stats.nodes));
    allMatch = false;
  }
  if (stats.captures != expectedStats->captures) {
    Reporter::printMessage("  [FAIL] Captures: expected " +
                           std::to_string(expectedStats->captures) + ", got " +
                           std::to_string(stats.captures));
    allMatch = false;
  }
  if (stats.enPassant != expectedStats->enPassant) {
    Reporter::printMessage("  [FAIL] En Passant: expected " +
                           std::to_string(expectedStats->enPassant) + ", got " +
                           std::to_string(stats.enPassant));
    allMatch = false;
  }
  if (stats.castles != expectedStats->castles) {
    Reporter::printMessage("  [FAIL] Castles: expected " +
                           std::to_string(expectedStats->castles) + ", got " +
                           std::to_string(stats.castles));
    allMatch = false;
  }
  if (stats.promotions != expectedStats->promotions) {
    Reporter::printMessage("  [FAIL] Promotions: expected " +
                           std::to_string(expectedStats->promotions) +
                           ", got " + std::to_string(stats.promotions));
    allMatch = false;
  }
  if (stats.checks != expectedStats->checks) {
    Reporter::printMessage("  [FAIL] Checks: expected " +
                           std::to_string(expectedStats->checks) + ", got " +
                           std::to_string(stats.checks));
    allMatch = false;
  }
  if (stats.checkmates != expectedStats->checkmates) {
    Reporter::printMessage("  [FAIL] Checkmates: expected " +
                           std::to_string(expectedStats->checkmates) +
                           ", got " + std::to_string(stats.checkmates));
    allMatch = false;
  }
