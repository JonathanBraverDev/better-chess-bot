...
constexpr int MOVE_FLAGS_LENGTH =
    4; // flags don't need individual offsets, only masks

static_assert(ORIGIN_INDEX_LENGTH + DESTINATION_INDEX_LENGTH +
                      CASTLE_RIGHTS_FLAGS + VALID_EN_PASSANT_LENGTH +
                      EN_PASSANT_INDEX_LENGTH + PIECE_TYPE_LENGTH +
                      CAPTURED_TYPE_LENGTH + MISC_MOVE_TYPE_LENGTH +
                      MOVE_FLAGS_LENGTH ==
                  32,
              "Total move encode length is wrong");

constexpr int ORIGIN_INDEX_OFFSET = 0;
constexpr int DESTINATION_INDEX_OFFSET =
    ORIGIN_INDEX_OFFSET + ORIGIN_INDEX_LENGTH;
constexpr int CASTLE_RIGHTS_FLAGS_OFFSET =
    DESTINATION_INDEX_OFFSET + DESTINATION_INDEX_LENGTH;
constexpr int VALID_EN_PASSANT_OFFSET =
    CASTLE_RIGHTS_FLAGS_OFFSET + CASTLE_RIGHTS_FLAGS;
constexpr int EN_PASSANT_INDEX_OFFSET =
    VALID_EN_PASSANT_OFFSET + VALID_EN_PASSANT_LENGTH;
constexpr int PIECE_TYPE_OFFSET =
    EN_PASSANT_INDEX_OFFSET + EN_PASSANT_INDEX_LENGTH;
constexpr int CAPTURED_TYPE_OFFSET = PIECE_TYPE_OFFSET + PIECE_TYPE_LENGTH;
constexpr int MISC_MOVE_TYPE_OFFSET =
    CAPTURED_TYPE_OFFSET + CAPTURED_TYPE_LENGTH;
constexpr int MOVE_FLAGS_OFFSET = MISC_MOVE_TYPE_OFFSET + MISC_MOVE_TYPE_LENGTH;

static_assert(MOVE_FLAGS_OFFSET + MOVE_FLAGS_LENGTH == 32,
              "Total move encode offset is wrong");
...
constexpr BitMove CASTLE_RIGHTS_FLAGS_MASK = 0b1111
                                             << CASTLE_RIGHTS_FLAGS_OFFSET;
...
static_assert((ORIGIN_INDEX_MASK | DESTINATION_INDEX_MASK |
               CASTLE_RIGHTS_FLAGS_MASK | VALID_EN_PASSANT_MASK |
               EN_PASSANT_INDEX_MASK | PIECE_TYPE_MASK | CAPTURED_TYPE_MASK |
               MISC_MOVE_TYPE_MASK | MOVE_FLAGS_MASK) == UINT_MAX,
              "Total move encode mask is wrong");
...
static_assert((IS_PROMOTE_MASK | IS_CHECK_MASK | IS_CAPTURE_MASK |
               FREE_FLAG_MASK) == 0b11110000000000000000000000000000,
              "Move ordering flag masks are wrong");

constexpr BitMove WHITE_SHORT_CASTLE_MASK = 0b1000
                                            << CASTLE_RIGHTS_FLAGS_OFFSET;
constexpr BitMove WHITE_LONG_CASTLE_MASK = 0b0100 << CASTLE_RIGHTS_FLAGS_OFFSET;
constexpr BitMove BLACK_SHORT_CASTLE_MASK = 0b0010
                                            << CASTLE_RIGHTS_FLAGS_OFFSET;
constexpr BitMove BLACK_LONG_CASTLE_MASK = 0b0001 << CASTLE_RIGHTS_FLAGS_OFFSET;

static_assert((WHITE_SHORT_CASTLE_MASK | WHITE_LONG_CASTLE_MASK |
               BLACK_SHORT_CASTLE_MASK | BLACK_LONG_CASTLE_MASK) ==
                  CASTLE_RIGHTS_FLAGS_MASK,
              "BitRights castle flags are wrong");

constexpr BitMove BIT_RIGHTS_MASK = 0b11111111 << CASTLE_RIGHTS_FLAGS_OFFSET;

static_assert((CASTLE_RIGHTS_FLAGS_MASK | VALID_EN_PASSANT_MASK |
               EN_PASSANT_INDEX_MASK) == BIT_RIGHTS_MASK,
              "BitRights mask is wrong");

// BitMove encoding:
// 6 bit origin index - must
// 6 bit destination index - must
// 3 bit moving/promoted type - must, logic help. when promote flag is on look
// as promotion 3 bit captured type - logic help total 18 bit move
