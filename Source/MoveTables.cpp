...
void MoveTables::prepareKnightMoves() {
  ...
  for (size_t i = 0; i < 64; i++) {
    knight_moves[i] = combineBoards(knight.look(Direction::KNIGHT_UP_LEFT),
...
void MoveTables::prepareWhitePawnMoves() {
  ...
  for (size_t i = 0; i < 64 * 2; i += 2) {
    ...
    pawn_attacks[i] = combineBoards(white_pawn.look(Direction::UP_LEFT),
...
void MoveTables::prepareBlackPawnMoves() {
  ...
  for (size_t i = 1; i < 64 * 2; i += 2) {
    ...
    pawn_attacks[i] = combineBoards(black_pawn.look(Direction::DOWN_LEFT),
