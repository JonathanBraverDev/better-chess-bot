...
void Move::fullClear() { encodedMove = 0; }

BitMove Move::getEncodedMove() const { return encodedMove; }
...
PieceType Move::getMovingOrPromotedType() const {
  PieceType type = static_cast<PieceType>((encodedMove & PIECE_TYPE_MASK) >>
                                          PIECE_TYPE_OFFSET);
...
AttackerType Move::getAttackerType() const {
  AttackerType attacker_type = static_cast<AttackerType>(
      (encodedMove & PIECE_TYPE_MASK) >> PIECE_TYPE_OFFSET);
...
PieceType Move::getAbsoluteMovingType() const {
  if (!isCapture() || isPromotion()) { // promotions use the regular types to
                                       // aid with move ordering
...
PieceType Move::getCapturedType() const {
  PieceType type = static_cast<PieceType>((encodedMove & CAPTURED_TYPE_MASK) >>
                                          CAPTURED_TYPE_OFFSET);
  assert(type != PieceType::NONE && type != PieceType::KING);
  return type;
}

MoveType Move::getMiscMoveType() const {
  return static_cast<MoveType>((encodedMove & MISC_MOVE_TYPE_MASK) >>
                               MISC_MOVE_TYPE_OFFSET);
}

bool Move::isCapture() const { return (encodedMove & IS_CAPTURE_MASK) != 0; }

bool Move::isCheck() const { return (encodedMove & IS_CHECK_MASK) != 0; }

bool Move::isPromotion() const { return (encodedMove & IS_PROMOTE_MASK) != 0; }

// for all setters, wipe the target data with the inverted mask and set the
// requested value
void Move::setOriginIndex(BoardIndex index) {
  assert(index <= 63);
...
bool Move::isNull() const { return (encodedMove & ~BIT_RIGHTS_MASK) == 0; }
